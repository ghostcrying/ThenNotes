...
```
codesign --deep --force --timestamp --verbose --options runtime --entitlements "$ENTITLEMENTS_PATH" --sign "3rd Party Mac Developer Application..." "$COMPONENT_APP_PATH"
```

### 开发者签名

```
codesign --force --timestamp --deep --options runtime --entitlements "$ENTITLEMENTS_PATH" --sign "Developer ID Application: ..." "$COMPONENT_APP_PATH"
...
find "$APP_PATH/Contents/MonoBundle" | while read -r MonoBundle_Files_Name; do
    if [[ -f "$MonoBundle_Files_Name" && "${MonoBundle_Files_Name##*.}" = "dylib" ]];then
        # 移除可能存在的旧签名并重新签名
        codesign --remove-signature "$MonoBundle_Files_Name" 2>/dev/null
        echo "[INFO] Signing $MonoBundle_Files_Name"
        codesign --force --timestamp --sign "$SIGNING_IDENTITY" "$MonoBundle_Files_Name"
    fi
done
```
...
```
# 打包
pkgbuild --component "$COMPONENT_APP_PATH" --identifier "com.xxx" --version 6.0 /Applications  "$PRODUCT_PKG_PATH"
- .app: 原始包目录
...
productsign --sign "Developer ID Installer: Name (yourTeamID)" "$PRODUCT_PKG_PATH" "$PRODUCT_SIGN_PKG_PATH"
```

##### 打包&签名

```
xcrun productbuild --sign "Developer ID Installer: Name (yourTeamID)" --component "$COMPONENT_APP_PATH" /Applications "$PRODUCT_SIGN_PKG_PATH"
...
- https://blog.csdn.net/qq_45489600/article/details/145309923
